...
class StimSettingCsvColumn(Enum):
    """
...
def get_stim_settings(dataset_path: Path) -> Sequence[StimSetting]:
    ...
    return [
        StimSetting(
            stim_option=row[StimSettingCsvColumn.STIM_OPTION.value],
            pulse_width=row[StimSettingCsvColumn.PULSE_WIDTH.value],
            current=row[StimSettingCsvColumn.CURRENT.value],
            frequency=row[StimSettingCsvColumn.FREQUENCY.value],
            duty_cycle_off=row[StimSettingCsvColumn.DUTY_CYCLE_OFF.value],
        )
